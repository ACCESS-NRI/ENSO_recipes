...
def plot_level1(input_data, cfg): #input data is 2 - model and obs
    ...
    plt.text(0.5, 0.95, f"RMSE: {rmse:.2f} {cube.units}", fontsize=12, ha='center', transform=plt.gca().transAxes,
    ...
    logger.info(f"{dataset['long_name']} {dataset['variable_group']} : rmse:{rmse}")
...
def main(cfg):
    ...
    for grp in variable_groups:
        # create pairs {'obs':} #get obs, pairs[mod].append(attributes)
